...
const toType = (node) => {
  ...
  if (node.type === "object") {
    ...
    const fields = Object.entries(node.properties ?? {}).map(([key, value]) =>
      `  ${JSON.stringify(key)}${required.has(key) ? "" : "?"}: ${toType(value)};`);
...
const definitions = Object.entries(schema.$defs).map(([name, definition]) =>
  `export type ${name} = ${toType(definition)};`);
const rootMessage = `export type ClientProtocolMessage = ${schema.oneOf.map(toType).join(" | ")};`;
const output = [
