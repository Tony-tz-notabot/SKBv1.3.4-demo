...
const chooseCard=(ref:string)=>{const spec=cardSpecFor(ref);if(spec){choose(spec,ref);return;}const offer=props.snapshot.interaction.offers.find((item)=>item.sourceRefs?.includes(ref)&&item.selectionSpecs.length===0);if(offer)chooseOffer(offer);};
