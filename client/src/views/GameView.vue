...
// PresentationEvent（含 eventType/payload）结构上与 StageEvent 一致；operationId 待服务端补齐①后透传。
// 必须用 computed：事件队列随快照/演示事件变化，setup 期一次性取值会拿到陈旧引用。
const stageEvents = computed(() => props.events as unknown as StageEvent[]);
...
<template>
  <section class="game-layout">
    ...
    <div class="game-table">
      ...
      <CombatStage :key="snapshot.stateRevision" :events="stageEvents" :draw-pile-count="snapshot.publicView.drawPileCount" :discard-top="snapshot.publicView.discardTop" :central-cards="snapshot.publicView.centralCards" :character-name="stageCharacterName" :relationship-cls="relationshipCls" :seat-position="seatPosition" />
    </div>
