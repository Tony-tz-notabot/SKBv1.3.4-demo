...
const playerPosition = (seat: Seat) => positions[(seat - props.snapshot.viewer.seat + 4) % 4];
...
const targetSpec = computed(() => activeOffer.value?.selectionSpecs.find((spec) => spec.kind === "targets"));
...
const legalTargetRefs = computed(() => new Set(targetSpec.value?.legalRefs ?? activeOffer.value?.legalTargetRefs ?? []));
...
const chooseCard=(ref:string)=>choose(cardSpecFor(ref),ref);
...
<template>
  <section class="game-layout">
    ...
    <div class="game-table">
      <div v-for="position in positions" :key="position" class="game-player-position" :data-position="position"><GamePlayerPanel v-if="playerAt(position)" :player="playerAt(position)!" :active="playerAt(position)!.seat === snapshot.publicView.activeSeat" :local="playerAt(position)!.seat === snapshot.viewer.seat" :legal-target="legalTargetRefs.has(`public:seat_${playerAt(position)!.seat}`)" :selected-target="selectedRefs.has(`public:seat_${playerAt(position)!.seat}`)" :selected-order="selectionOrder(targetSpec?.key,`public:seat_${playerAt(position)!.seat}`)" :preselected-weapon-slot="playerAt(position)!.seat===snapshot.viewer.seat?snapshot.privateView.preselectedWeaponSlot:null" :preselectable-weapon-slots="playerAt(position)!.seat===snapshot.viewer.seat?snapshot.privateView.preselectableWeaponSlots:[]" :legal-card-refs="legalCardRefs" :selected-card-refs="selectedRefs" @select="choose(targetSpec,$event)" @card-select="chooseCard" @slot-select="selectEquipmentSlot" @detail="detailCard=$event" /></div>
