...
function offerLabel(offer: DeepReadonly<InteractionOffer>) {
  const generic={ declareAttack:"发动攻击",useCard:"使用卡牌",activateAbility:"发动技能",respond:"响应",equip:"装备",discardEquipment:"丢弃装备",dismantle:"拆除",synthesize:"合成",interveneJudgment:"干预判定",rescueDying:"救援",resolveChoice:"确认选择",pass:"放弃",endPhase:"结束阶段" } as Record<string,string>;
  ...
  if(cardName){if(offer.kind==="equip")return`装备${cardName}`;if(offer.kind==="discardEquipment")return`弃置${cardName}`;if(offer.kind==="dismantle")return`拆除${cardName}`;if(offer.kind==="synthesize")return`合成${cardName}`;if(offer.kind==="useCard")return`使用${cardName}`;}
...
function selectEquipmentSlot(slotId:string,card:DeepReadonly<CardView>|null){const offers=props.snapshot.interaction.offers,equipFor=()=>offers.find((o)=>o.kind==="equip"&&equipSlotOf(o)===slotId);if((slotId==="armor"||/^talent:\d:/.test(slotId))&&equipFor()){chooseOffer(equipFor()!);return;}if(!props.snapshot.privateView.preselectableWeaponSlots.includes(slotId))return;emit("preselection",slotId,card?.state.modeId??null);}
