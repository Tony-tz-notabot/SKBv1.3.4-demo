...
import MatchLogPanel from "../components/MatchLogPanel.vue";
...
// 中央战斗区（CombatStage）：本地玩家恒右下、其余逆时针——与 playerPosition 同布局，映射到舞台逻辑坐标。
// 注意：CombatStage 舞台坐标空间为 600×500（VIEW_W/VIEW_H），此处小数（0~1）必须乘上舞台尺寸，
// 否则箭头路径退化为几像素的点、肉眼不可见。
const STAGE_W = 600;
const STAGE_H = 500;
const STAGE_POSITIONS: readonly [number, number][] = [[0.8, 0.88], [0.8, 0.12], [0.2, 0.12], [0.2, 0.88]];
const seatPosition = (seat: number): Point => {
  const offset = (seat - (viewerSeat.value ?? 1) + 4) % 4;
  const [x, y] = STAGE_POSITIONS[offset]!;
  return { x: x * STAGE_W, y: y * STAGE_H };
};
// 词条关系色：自己 rel-self / 队友 rel-ally / 对手 rel-enemy；观战 null。
const relationshipCls = (seat: number | null): string | null => {
  if (seat === null || props.snapshot.viewer.seat == null) return null;
  if (seat === props.snapshot.viewer.seat) return "rel-self";
  const target = props.snapshot.publicView.players.find((p) => p.seat === seat);
  return target && target.team === props.snapshot.viewer.team ? "rel-ally" : "rel-enemy";
};
const stageCharacterName = (seat: number | null): string => (seat == null ? "系统" : characterNameOfSeat(seat));
// 事件源：快照携带的按观众过滤演示事件（activityEvents）——实时 PRESENTATION_EVENT 队列会因
// "快照先重置 lastEventSeq、随后事件 seq≤lastEventSeq" 被全部丢弃（服务端广播序 + 客户端去重不匹配）。
// 快照携带事件历史（最近 200 条）随 stateRevision 变化整体刷新，CombatStage 依 :key 重放重建操作组。
const stageEvents = computed(() => (props.snapshot.activityEvents ?? props.events) as unknown as StageEvent[]);
// A2/A4：标题与 ROUND 随内容变化做淡入过渡（130 批 2）
const headlineKey = computed(() => `${props.snapshot.publicView.headline ?? ""}|${props.snapshot.publicView.activeSeat}|${props.snapshot.publicView.phase}`);
...
<template>
  <section class="game-layout">
    ...
    <header class="game-hud"><div><Transition name="round-pop" mode="out-in"><p class="eyebrow" :key="snapshot.publicView.round">ROUND {{ snapshot.publicView.round }}</p></Transition><Transition name="hud-title" mode="out-in"><h2 :key="headlineKey">{{ snapshot.publicView.headline ?? `${snapshot.publicView.activeSeat}号玩家行动` }}</h2></Transition></div><div class="phase-track"><span v-for="phase in ['prepare','judgment','draw','play','discard','end']" :key="phase" :class="{ active: snapshot.publicView.phase === phase }">{{ phase }}</span></div><div class="game-hud__actions"><button v-if="canDisbandRoom" type="button" class="button button--danger" @click="confirmDisband">解散房间</button><button v-if="!snapshot.publicView.headline" type="button" class="forfeit-button" @click="confirmLeaveGame">退出本局</button></div></header>
    <div class="game-table">
      ...
      <CombatStage :key="snapshot.gameId" :events="stageEvents" :draw-pile-count="snapshot.publicView.drawPileCount" :discard-top="snapshot.publicView.discardTop" :central-cards="snapshot.publicView.centralCards" :character-name="stageCharacterName" :relationship-cls="relationshipCls" :seat-position="seatPosition" />
    </div>
    ...
    <div class="game-activity"><MatchLogPanel :snapshot="snapshot"/><GameChatPanel :messages="snapshot.chat" @send="forwardChat"/></div>
