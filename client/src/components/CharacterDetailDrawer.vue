  ...
  <Teleport to="body">
    <div v-if="candidate" class="detail-backdrop" role="presentation" @click.self="emit('close')">
      <aside class="detail-drawer" role="dialog" aria-modal="true" :aria-label="`${candidate.displayName}详情`">
        ...
        <section><h3>初始天赋</h3><p>{{ candidate.initialTalentId }}</p></section>
        <section><h3>角色技能</h3><ul><li v-for="abilityId in candidate.abilityIds" :key="abilityId">{{ abilityId }}</li></ul></section>
