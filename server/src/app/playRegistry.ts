...
import {buildWeaponChargeOffers,WeaponChargeSession} from "../engine/weaponCharge.js";
import {buildWeaponSpecialOffers,WeaponSpecialSession} from "../engine/weaponSpecialOffers.js";
import {BossUseCommandSession,legalStandardPersistentBossCards} from "../engine/bossUse.js";
import {C6BombardmentUseCommandSession,legalC6BombardmentUses,C6LaserSweepUseCommandSession,legalC6LaserSweepUses} from "../engine/c6h8o6.js";
...
standard(buildWeaponChargeOffers,WeaponChargeSession),
{offers:(s,_r,n)=>as(buildWeaponSpecialOffers(s,n)),session:(s,r)=>new WeaponSpecialSession(s,r)},
{offers:(s,r,n)=>{try{return legalStandardPersistentBossCards(s,r,n).map(cardRef=>({offerId:`offer:boss-use:${cardRef}`,kind:"useCard",cardRef}))}catch{return[]}},session:(s,r)=>new BossUseCommandSession(s,r)},
{offers:(s,r,n)=>legalC6LaserSweepUses(s,r,n).map(cardRef=>({offerId:`offer:boss-use:c6-sweep:${cardRef}`,kind:"useCard",cardRef,modeOptions:["kill","dodge"]})),session:(s,r)=>new C6LaserSweepUseCommandSession(s,r)},
{offers:(s,r,n)=>legalC6BombardmentUses(s,r,n).map(u=>({offerId:`offer:boss-use:c6-bomb:${u.cardRef}`,kind:"useCard",cardRef:u.cardRef,modeOptions:["kill","dodge"],legalTargetRefs:u.legalTargetRefs,targetMin:1,targetMax:1})),session:(s,r)=>new C6BombardmentUseCommandSession(s,r)},
];
