 ...
 handle(room:AppRoom,user:AppUser,command:Command):GameResult{
  ...
  try{
   ...
   if(room.game.lifecycle==="setupRedraw"){
    ...
    if(!out.accepted){const result={accepted:false as const,commandId:out.commandId,reasonCode:out.reasonCode,messageKey:out.messageKey,stateRevision:out.stateRevision,refreshRequired:out.refreshRequired};this.results.set(command.commandId,result);return result}room.game=session.state;this.stabilize(room);if(room.game.lifecycle==="ended"){this.disband(room);return this.accept(command,room,out.firstEventSeq??room.game.lastEventSeq)}return this.accept(command,room,out.firstEventSeq??room.game.lastEventSeq);
   }
   ...
   if(window){
    ...
    if(session){const out=(session as any).handle({...base,...(window.kind==="attackResponse"||window.kind==="dyingRescue"?{cardRef:cards[0]}:{}),...(window.kind==="preJudgment"?{cardRefs:cards}:{})});if(!out.accepted)return reject(out.reasonCode,out.refreshRequired);room.game=(session as any).state;this.stabilize(room);if(room.game!.lifecycle==="ended"){this.disband(room);return this.accept(command,room,out.events?.[0]?.eventSeq??room.game!.lastEventSeq)}return this.accept(command,room,out.events?.[0]?.eventSeq??room.game!.lastEventSeq);}
   }
   if(window&&(window.kind==="playPhaseAction"||window.kind==="discardPhaseAction")&&(command.offerId.includes("finish")||window.kind==="discardPhaseAction")){
    ...
    if(!out.accepted)return reject(out.reasonCode,out.refreshRequired);room.game=session.state;this.stabilize(room);if(room.game.lifecycle==="ended"){this.disband(room);return this.accept(command,room,out.firstEventSeq??room.game.lastEventSeq)}return this.accept(command,room,out.firstEventSeq??room.game.lastEventSeq);
   }
