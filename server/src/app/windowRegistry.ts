...
const standard=(Class:new(...args:any[])=>unknown):Factory=>(s,r,d)=>{const session=construct(Class,[s,r,d]);return{get state(){return session.state},handle:c=>session.handle(c),handleTimeout:typeof session.handleTimeout==="function"?(id:string)=>session.handleTimeout!(id):((commandId:string)=>{const w=s.pendingWindows[0];if(!w)throw new Error("PROMPT_CLOSED");const pass=w.legalOfferIds.find(id=>id.endsWith(":pass"))??w.legalOfferIds.find(id=>id.includes(":pass:"));let offerId=pass??w.legalOfferIds[0]!;if(!offerId)throw new Error("NO_TIMEOUT_OFFER");if(!pass&&w.timeoutPolicy==="randomLegal"&&w.legalOfferIds.length>1){const candidates=[...w.legalOfferIds],randomSeq=s.randomSource.nextRandomSeq,chosen=chooseWithSource(candidates,s.randomSource);s.randomSource=chosen.source;s.randomHistory.push({randomSeq,purpose:`timeout.${w.kind}`,candidateRefs:candidates,resultRefs:[chosen.value]});offerId=chosen.value;}const actor=s.players.find(p=>p.seat===w.prioritySeat);if(!actor)throw new Error("NO_TIMEOUT_ACTOR");try{return session.handle({commandId,gameId:s.gameId,expectedStateRevision:s.stateRevision,actorUserId:actor.userId,promptId:w.promptId,offerId});}catch{return{accepted:false as const,commandId,stateRevision:s.stateRevision,reasonCode:"TIMEOUT_CONTEXT_INVALID",refreshRequired:true};}})};};
