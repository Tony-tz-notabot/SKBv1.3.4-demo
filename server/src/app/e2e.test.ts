...
async function snapshotFor(clients:TestClient[],channel:"room"|"game",predicate:(message:any)=>boolean){const start=starts(clients);return waitFor(clients,start,entry=>entry.type==="MESSAGE"&&entry.channel===channel&&predicate(entry.message));}
...
describe("real websocket four-player E2E",()=>{
 it("reaches a legal winner through real room, redraw, attack, response and elimination commands",async()=>{const dir=await mkdtemp(join(tmpdir(),"skb-e2e-")),persistence=new JsonPersistence(join(dir,"state.json")),rooms=new RoomService(ruleset,persistence);dirs.push(dir);await rooms.restore();const server=new SkbApplicationServer(rooms,ruleset);await server.listen(0);const port=(server.http.address() as AddressInfo).port,base=`http://127.0.0.1:${port}`;
  ...
  try{
   ...
   for(let i=0;i<4;i++){const latest=latestRoom(clients[i]!),candidates=selectionSnaps[i]!.characterSelection.candidates,characterId=i===0?(candidates.find(candidate=>candidate.characterId!=="character.shaman")??candidates[0]!).characterId:candidates[0]!.characterId;await command(clients[i]!,{type:"ROOM_COMMAND",commandId:`lock${i}`,roomId:latest.roomId,expectedRoomRevision:latest.roomRevision,command:"LOCK_CHARACTER",payload:{characterId}},"room");}
   ...
   const current=rooms.roomForUser(sessions[0]!.userId)!.game!,snap=new GameProjector(ruleset).game(rooms.roomForUser(sessions[0]!.userId)!,sessions[0]!.userId) as any;console.log("OFFERS",JSON.stringify(snap.interaction.offers.map((offer:any)=>offer.kind)));
   for(const targetSeat of[2,3]as const){
    const hostSnap=(await waitFor(clients,clients.map(client=>({client,index:0})),entry=>entry.type==="MESSAGE"&&entry.channel==="game"&&entry.message.type==="GAME_SNAPSHOT"&&entry.message.viewer.seat===1&&entry.message.interaction.offers.some((offer:any)=>offer.kind==="declareAttack"))).message,offer=hostSnap.interaction.offers.find((item:any)=>item.kind==="declareAttack"),killSpec=offer.selectionSpecs.find((spec:any)=>spec.key==="killCards")??offer.selectionSpecs.find((spec:any)=>spec.kind==="cards"),killRef=killSpec?.legalRefs?.[0];
