...
let drainSeq=0;
async function drainWindows(clients:TestClient[],room:NonNullable<ReturnType<RoomService["roomForUser"]>>,gameId:string){
 for(let guard=0;guard<60;guard+=1){
  if(!room.game||room.game.lifecycle==="ended")return;
...
describe("real websocket four-player E2E",()=>{
 it("reaches a legal winner through real room, redraw, attack, response and elimination commands",async()=>{const dir=await mkdtemp(join(tmpdir(),"skb-e2e-")),persistence=new JsonPersistence(join(dir,"state.json")),rooms=new RoomService(ruleset,persistence);dirs.push(dir);await rooms.restore();const server=new SkbApplicationServer(rooms,ruleset);await server.listen(0);const port=(server.http.address() as AddressInfo).port,base=`http://127.0.0.1:${port}`;
  ...
  try{
   ...
   for(let i=0;i<4;i++){const latest=latestRoom(clients[i]!),candidates=selectionSnaps[i]!.characterSelection.candidates as Array<{characterId:string}>,avoid=["character.punching_bag","character.interdimensional_traveler",...(i===0?["character.shaman","character.berserker"]:[])],pick=candidates.find(candidate=>!avoid.includes(candidate.characterId))??candidates[0]!;await command(clients[i]!,{type:"ROOM_COMMAND",commandId:`lock${i}`,roomId:latest.roomId,expectedRoomRevision:latest.roomRevision,command:"LOCK_CHARACTER",payload:{characterId:pick.characterId}},"room");}
   ...
   const kills=Object.values(game.cards).filter(card=>card.templateId.startsWith("basic.kill.")).slice(0,2);for(const card of kills){const from=game.zones[card.zoneRef]!;if(from.zoneRef!=="hand:1"){from.orderedCardRefs.splice(from.orderedCardRefs.indexOf(card.cardRef),1);game.zones["hand:1"]!.orderedCardRefs.push(card.cardRef);Object.assign(card,{zoneRef:"hand:1",ownerSeat:1,controllerSeat:1,faceUp:false});}}
   ...
   for(const targetSeat of[2,3]as const){
    const loopRoom=room.game!;
    let playWindow=loopRoom.pendingWindows.find((w:any)=>w.kind==="playPhaseAction"&&w.prioritySeat===1);
    for(let guard=0;!playWindow&&guard<600;guard+=1){await delay(20);playWindow=room.game!.pendingWindows.find((w:any)=>w.kind==="playPhaseAction"&&w.prioritySeat===1);}
    ...
    const attackResult=await command(clients[0]!,{type:"GAME_COMMAND",commandId:`attack${targetSeat}`,gameId:hostSnap.gameId,expectedStateRevision:room.game!.stateRevision,promptId:hostSnap.interaction.prompt.promptId,offerId:offer.offerId,command:"EXECUTE_OFFER",payload:{selections:{killCards:[killRef],targets:[`public:seat_${targetSeat}`]}}},"game");lastGameRevision=attackResult.stateRevision;
    ...
    const passResult=await command(clients[targetIndex]!,{type:"GAME_COMMAND",commandId:`pass${targetSeat}`,gameId:responseSnap.gameId,expectedStateRevision:room.game!.stateRevision,promptId:responseSnap.interaction.prompt.promptId,offerId:pass.offerId,command:"EXECUTE_OFFER",payload:{selections:{}}},"game");lastGameRevision=passResult.stateRevision;
    await drainWindows(clients,room,responseSnap.gameId);lastGameRevision=room.game!.stateRevision;
   }
   // 对局结束（正常胜利）后房间解散：玩家与观战者被移出、房间 closed，客户端回到大厅。
   ...
   // 观战者与玩家一样被送回大厅：收到对局结束后的 LOBBY_SNAPSHOT
   const spectatorLobby=clients[4]!.messages.filter(entry=>entry.type==="MESSAGE"&&entry.channel==="room"&&entry.message.type==="LOBBY_SNAPSHOT").at(-1);
   expect(spectatorLobby?.message.type).toBe("LOBBY_SNAPSHOT");
 ...
 it("serves no static files by default (server-only mode saves bandwidth)",async()=>{const dir=await mkdtemp(join(tmpdir(),"skb-static-off-")),persistence=new JsonPersistence(join(dir,"state.json")),rooms=new RoomService(ruleset,persistence);dirs.push(dir);await rooms.restore();const server=new SkbApplicationServer(rooms,ruleset,{}, {serveStatic:false});await server.listen(0);const port=(server.http.address() as AddressInfo).port;try{const response=await fetch(`http://127.0.0.1:${port}/`);expect(response.status).toBe(404);const img=await fetch(`http://127.0.0.1:${port}/assets/index-abc.js`);expect(img.status).toBe(404);}finally{await server.close();}});
 it("serves the client app when static hosting is enabled",async()=>{const dir=await mkdtemp(join(tmpdir(),"skb-static-on-")),persistence=new JsonPersistence(join(dir,"state.json")),rooms=new RoomService(ruleset,persistence);dirs.push(dir);await rooms.restore();const server=new SkbApplicationServer(rooms,ruleset,{}, {serveStatic:true});await server.listen(0);const port=(server.http.address() as AddressInfo).port;try{const response=await fetch(`http://127.0.0.1:${port}/`);expect(response.status).toBe(200);expect(await response.text()).toContain("<!doctype html>");}finally{await server.close();}});
 it("allows cross-origin API calls from the offline client (CORS)",async()=>{const dir=await mkdtemp(join(tmpdir(),"skb-cors-")),persistence=new JsonPersistence(join(dir,"state.json")),rooms=new RoomService(ruleset,persistence);dirs.push(dir);await rooms.restore();const server=new SkbApplicationServer(rooms,ruleset,{}, {serveStatic:false});await server.listen(0);const port=(server.http.address() as AddressInfo).port,base=`http://127.0.0.1:${port}`,origin="http://localhost:8080";try{const preflight=await fetch(`${base}/api/session`,{method:"OPTIONS",headers:{origin,"access-control-request-method":"POST","access-control-request-headers":"content-type"}});expect(preflight.status).toBe(204);expect(preflight.headers.get("access-control-allow-origin")).toBe("*");expect(preflight.headers.get("access-control-allow-headers")).toContain("content-type");const login=await fetch(`${base}/api/session`,{method:"POST",headers:{"content-type":"application/json",origin},body:JSON.stringify({username:"cors1",password:"x"})});expect(login.status).toBe(200);expect(login.headers.get("access-control-allow-origin")).toBe("*");const health=await fetch(`${base}/health`,{headers:{origin}});expect(health.headers.get("access-control-allow-origin")).toBe("*");}finally{await server.close();}});
});
